  ...
  try {
    if (request.session.loggedIn) {
      ...
      if (request.body.guess === check.rows[0].answer) {
        if (check.rows[0].id < 25) {
        ...
        } else if (check.rows[0].id >= 25 && check.rows[0].id < 40) {
          const selectl = await selectflag(request.session.rollno);

          if (selectl.rows[0].flag < 40 && selectl.rows[0].flag >= 1) {
            score = 300 - selectl.rows[0].flag * 5;
            const updatel = await updateflag(request.session.rollno);
            const updatescore = await correctanswer(
              score,
              request.session.rollno
          ...
          } else if (selectl.rows[0].flag >= 40) {
            const updatel = await updateflag(request.session.rollno);
            const updatefirst_login = await updatefirstanswer(
              100,
              request.session.rollno
          ...
          if (selectl.rows[0].flag < 125 && selectl.rows[0].flag >= 1) {
            score = 400 - selectl.rows[0].flag * 2;
            const updatel = await updateflag(request.session.rollno);
            const updatescore = await correctanswer(
              score,
              request.session.rollno
          ...
          } else if (selectl.rows[0].flag >= 125) {
            const updatel = await updateflag(request.session.rollno);
            const updatefirst_login = await updatefirstanswer(
              150,
              request.session.rollno
